...
from .embedding import BPESentenceEmbedding, SPMSentenceEmbedding
        ...
        if mode == 'spm':
            if spm_model is None:
                if not os.path.isfile(self.DEFAULT_SPM_MODEL_FILE):
                    raise FileNotFoundError(
                        'laser2.spm is missing, run "python -m laserembeddings download-models" to fix that'
                    )
                spm_model = self.DEFAULT_SPM_MODEL_FILE
            if spm_vocab is None:
                if not os.path.isfile(self.DEFAULT_SPM_VOCAB_FILE):
                    raise FileNotFoundError(
                        'laser2.cvocab is missing, run "python -m laserembeddings download-models" to fix that'
                    )
                spm_vocab = self.DEFAULT_SPM_VOCAB_FILE
            if encoder is None:
                if not os.path.isfile(self.DEFAULT_ENCODER_LASER2_FILE):
                    raise FileNotFoundError(
                        'laser2.pt is missing, run "python -m laserembeddings download-models" to fix that'
                    )
